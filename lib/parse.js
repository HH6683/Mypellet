...
if (type === 'Booking Confirmation') {
  ...
  // —— Booking No —— 
  let booking = /Booking\s*No\.?\s*:\s*(\S+)/i.exec(text)?.[1] || '';
  if (!booking) {
    // 新格式可能写成 “Booking No XXXXX”
    booking = /Booking\s*No\s*([A-Z0-9]+)/i.exec(text)?.[1] || '';
  }

  // —— Vessel/Voyage —— 
  let vessel = /Vessel\s*\/\s*Voyage\s*:\s*([^\r\n]+)/i.exec(text)?.[1].trim() || '';
  if (!vessel) {
    // 备选匹配 “VSL Name / Voy XXXXX”
    vessel = /VSL\s*Name\s*\/\s*Voy\s*[:\s]*([^\r\n]+)/i.exec(text)?.[1]?.trim() || '';
  }

  // —— POL —— 
  let pol = /POL\s*:\s*([^\r\n]+)/i.exec(text)?.[1]?.trim() || '';
  if (!pol) {
    // 备选匹配 “Port of Loading XXXXX”
    pol = /Port\s*of\s*Loading\s*([^\r\n]+)/i.exec(text)?.[1]?.trim() || '';
  }
