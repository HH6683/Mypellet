...
export async function parseFile(buffer, type, filename) {
 ...
 if (type === 'Booking Confirmation' && ext === '.pdf') {
    ...
    if (!pol) {
  // 新逻辑：先找包含“Port of Discharge”的行
  const pLine = linesAfter.find(l => /Port\s+of\s+Discharge/i.test(l));
  if (pLine) {
    // 截取“Port of Discharge”后，到“Port of Loading”前的部分
    const afterDischarge = pLine.split(/Port\s+of\s+Discharge/i)[1] || '';
    pol = (afterDischarge.split(/Port\s+of\s+Loading/i)[0] || '').trim();
  }
}
